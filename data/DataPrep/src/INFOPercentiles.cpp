...
void parse_bcf_record(bcf_hdr_t* header, bcf1_t* rec, bcf_header_summary& h, bcf_record_summary& r, bool qual) throw (runtime_error) {
   r.clear();
...
int main(int argc, char* argv[]) {
   ...
   bool qual = false;
	...
	desc.add_options()
      ...
      ("percentiles,p", po::value<unsigned int>(&n_percentiles)->default_value(100u), "Number of percentiles to compute. Default is 100.")
		...
		("out,o", po::value<string>(&output_prefix)->required(), "Prefix for output files. Output is compressed using gzip.")
	;
	...
	try {
		...
		if (vm.count("help")) {
			cout << "This program computes percentiles for the QUAL field or any arbitrary numeric INFO field across all variants." << endl;
   ...
   if (metric_name.length() == 0) {
	...
	try {
               ...
               parse_bcf_header(header, metric_name, header_summary, qual);
                  ...
                  if (qual) {
                     for (unsigned int i = 0u; i < record_summary.alts.size(); ++i) {
							   if ((allele_count != numeric_limits<double>::max()) && (allele_count != record_summary.ac_values[i])) {
								   continue;
							   }
							   maf = fcmp(record_summary.af_values[i], 0.5, 0.00000001) > 0 ? 1.0 - record_summary.af_values[i] : record_summary.af_values[i];
                  ...
                  } else if (record_summary.metric_values.size() == 1)  { // single metric value for all alt alleles
                     for (unsigned int i = 0u; i < record_summary.alts.size(); ++i) {
							   if ((allele_count != numeric_limits<double>::max()) && (allele_count != record_summary.ac_values[i])) {
								   continue;
							   }
                  ...
                  } else if (record_summary.metric_values.size() == record_summary.alts.size()) { // one metric value for every alt allele 
                     for (unsigned int i = 0u; i < record_summary.alts.size(); ++i) {
							   if ((allele_count != numeric_limits<double>::max()) && (allele_count != record_summary.ac_values[i])) {
								   continue;
							   }
      ...
      cout << "Merging... " << flush;
      Percentiles percentiles(qual ? "QUAL" : metric_name, qual ? "Phred-scaled site quality score" : metric_desc, percentiles_per_file);
      cout << "Done." << endl;
      ...
      string output_percentiles_file = output_prefix + ".all_percentiles.gz";
		writer.open(output_percentiles_file.c_str());
		percentiles.write(writer);
		writer.close();
      ...
      while (end < input_files.size()) {
			...
			threads.emplace_back([&](unsigned int start, unsigned int end) {
               ...
               GzipWriter variant_writer;
               variant_writer.open(output_file.c_str());
               variant_writer.write("##fileformat=VCFv4.3\n");
               if (qual) {
                  variant_writer.write("##INFO=<ID=QUAL_PCTL,Number=2,Type=Float,Description=\"Phred-scaled quality score for the assertion made in ALT\">\n");
               } else {
                  variant_writer.write("##INFO=<ID=%s_PCTL,Number=2,Type=Float,Description=\"%s\">\n", metric_name.c_str(), metric_desc.c_str());

               }
               variant_writer.write("#CHROM\tPOS\tID\tREF\tALT\tQUAL\tFILTER\tINFO\n");
					while (bcf_sr_next_line(sr) > 0) {
                  bcf1_t* rec = bcf_sr_get_line(sr, 0);
                  parse_bcf_record(header, rec, header_summary, record_summary, qual);
                  if (qual) {
                     ...
                     for (unsigned int i = 0u; i < record_summary.alts.size(); ++i) {
                        variant_writer.write("%s\t%lu\t%s\t%s\t%s\t.\t.\tQUAL_PCTL=%g,%g\n", 
                     ...
                     if (record_summary.metric_values.size() == 1) { // single metric for all alt alleles
                        ...
                        for (unsigned int i = 0u; i < record_summary.alts.size(); ++i) {
                           variant_writer.write("%s\t%lu\t%s\t%s\t%s\t.\t.\t%s_PCTL=%g,%g\n", 
                              record_summary.chrom, record_summary.position, record_summary.id,
