...
int main(int argc, char* argv[]) {
    ...
    desc.add_options()
            ...
            ("samples,s", po::value<string>(&samples_file), "Input file with samples. File must be tab-delimited (without header) and must have at least two columns: first - new sample name, second - current sample name used in VCF/BCF. New and current sample name may be the same if you don't wish to anonymize your samples.")
            ...
            ("out,o", po::value<string>(&output_prefix)->default_value("output"), "Prefix for output files (output is compressed using bgzip).")
            ;
    ...
    try {
        if (!samples_file.empty()) {
            samples = read_samples(samples_file.c_str(), 2);
        }
        map<string, string> sample_new_names = read_sample_names_map(samples_file.c_str(), 2, 1);
        ...
        string output_variants_file(output_prefix + ".variant_map.tsv.gz");
        BGZF *ofp = bgzf_open(output_variants_file.c_str(), "w");
        if (!ofp) {
        ...
        write(ofp, "#CHROM\tPOS\tREF\tALT\tHOM\tHET\n");
        ...
        while (bcf_sr_next_line(sr) > 0) {
            ...
            for (int i = 1; i < rec->n_allele; ++i) {
                ...
                if (hom_it != hom_samples.end()) {
                    auto n_hom = hom_it->second.size() > n_random ? n_random : hom_it->second.size();
                    shuffle(hom_it->second.begin(), hom_it->second.end(), number_generator);
                    for (unsigned int j = 0u; j < n_hom; ++j) {
                        ...
                        write(ofp, "%s%s", sample_new_names.at(header->samples[hom_it->second[j]]).c_str(), j < n_hom - 1 ? "," : "");
                    }
                }
                write(ofp, "\t");
                auto het_it = het_samples.find(i);
                if (het_it != het_samples.end()) {
                    auto n_het = het_it->second.size() > n_random ? n_random : het_it->second.size();
                    shuffle(het_it->second.begin(), het_it->second.end(), number_generator);
                    for (unsigned int j = 0u; j < n_het; ++j) {
                        ...
                        write(ofp, "%s%s", sample_new_names.at(header->samples[het_it->second[j]]).c_str(), j < n_het - 1 ? "," : "");
                    }
                }
                write(ofp, "\n");
            }
        ...
        string output_samples_file(output_prefix + ".sample_map.tsv.gz");
        ofp = bgzf_open(output_samples_file.c_str(), "w");
        if (!ofp) {
            throw runtime_error("Error while opening output file!");
        }
        write(ofp, "#RANDOM_SEED=%d\n", random_seed);
